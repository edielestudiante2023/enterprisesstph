...
class PtaclienteModel extends Model
{
    ...
    // Método para obtener registros filtrados por estado_actividad
    public function getActividadesByEstado($estado = null)
    {
        if ($estado) {
            return $this->where('estado_actividad', $estado)
                        ->orderBy('id_ptacliente', 'DESC')
                        ->findAll();
        }

        // Si no se especifica estado, retorna todos ordenados
        return $this->orderBy('id_ptacliente', 'DESC')->findAll();
    }
}
