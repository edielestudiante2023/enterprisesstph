<?php $SLUG = 'productos-quimicos'; $TITULO = 'Productos Quimicos'; $ICONO = 'fa-flask'; ?>
<div class="container-fluid px-3 mt-2">
    ...
    <table id="tablaInsp" class="table table-sm table-hover" style="width:100%">
        <thead><tr><th>#</th><th>Cliente</th><th>Fecha</th><th>Estado</th><th>% Cumpl.</th><th>Nivel</th><th>Acciones</th></tr></thead>
        <tbody>
        <?php foreach ($inspecciones as $i => $r):
            ...
            $nivelMap = ['alto'=>['Alto','#28a745'],'medio'=>['Medio','#ffc107'],'bajo'=>['Bajo','#dc3545']];
            $nivel = $r['nivel_riesgo'] ?? null;
        ...
        <tr>
            ...
            <td><?= $r['porcentaje_cumplimiento'] !== null ? number_format((float)$r['porcentaje_cumplimiento'],1).'%' : '-' ?></td>
            <td>
                <?php if ($nivelInfo): ?>
                    <span style="background:<?= $nivelInfo[1] ?>; color:#fff; padding:2px 6px; border-radius:4px; font-size:11px;"><?= $nivelInfo[0] ?></span>
                <?php else: ?>
                    -
                <?php endif; ?>
            </td>
