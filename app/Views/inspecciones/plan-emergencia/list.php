<div class="container-fluid px-3">
    ...
    <!-- Boton nuevo -->
    <a href="<?= base_url('/inspecciones/plan-emergencia/create') ?>" class="btn btn-pwa btn-pwa-primary mb-3">
        <i class="fas fa-plus"></i> Nuevo Plan de Emergencia
    </a>
    ...
    <?php if (empty($inspecciones)): ?>
        <div class="text-center text-muted py-4">
            <i class="fas fa-file-medical" style="font-size:40px; opacity:0.3;"></i>
            <p class="mt-2">No hay planes de emergencia</p>
        </div>
    <?php else: ?>
        ...
        <div class="card card-inspeccion mb-2 insp-card" data-cliente="<?= esc($insp['nombre_cliente'] ?? '') ?>">
            <div class="card-body py-2 px-3">
                <div class="d-flex justify-content-between align-items-start">
                    <div style="flex:1;">
                        ...
                        <div class="text-muted" style="font-size:12px;">
                            <?= date('d/m/Y', strtotime($insp['fecha_visita'])) ?>
                        </div>
                    </div>
                ...
                <div class="mt-2 d-flex gap-1 flex-wrap">
                        <a href="<?= base_url('/inspecciones/plan-emergencia/edit/') ?><?= $insp['id'] ?>" class="btn btn-sm btn-outline-dark">
                    ...
                    <?php if ($insp['estado'] === 'completo'): ?>
                        <a href="<?= base_url('/inspecciones/plan-emergencia/view/') ?><?= $insp['id'] ?>" class="btn btn-sm btn-outline-dark">
                            <i class="fas fa-eye"></i> Ver
                        </a>
                        <a href="<?= base_url('/inspecciones/plan-emergencia/pdf/') ?><?= $insp['id'] ?>" class="btn btn-sm btn-outline-primary" target="_blank">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        Swal.fire({
            title: 'Eliminar plan?',
            html: 'Se eliminara el Plan de Emergencia de <strong>' + btn.dataset.nombre + '</strong>',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                window.location.href = '<?= base_url('/inspecciones/plan-emergencia/delete/') ?>' + btn.dataset.id;
            }
        });
    });
});
