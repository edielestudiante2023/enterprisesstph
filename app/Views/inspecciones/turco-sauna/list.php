<?php $SLUG = 'turco-sauna'; $TITULO = 'Turco / Sauna / Jacuzzi'; $ICONO = 'fa-hot-tub-person'; ?>
<div class="container-fluid px-3 mt-2">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="mb-0"><i class="fas <?= $ICONO ?>"></i> <?= $TITULO ?> (FT-SST-249)</h6>
    ...
    <table id="tablaInsp" class="table table-sm table-hover" style="width:100%">
        <thead><tr><th>#</th><th>Cliente</th><th>Fecha</th><th>Recintos</th><th>Estado</th><th>Acciones</th></tr></thead>
        <tbody>
        <?php foreach ($inspecciones as $i => $r):
            ...
            $recintos = [];
            if (!empty($r['aplica_turco'])) $recintos[] = 'Turco';
            if (!empty($r['aplica_sauna'])) $recintos[] = 'Sauna';
            if (!empty($r['aplica_jacuzzi'])) $recintos[] = 'Jacuzzi';
        ?>
        <tr>
            <td><?= $i+1 ?></td>
            <td><?= esc($r['nombre_cliente']??'') ?></td>
            <td data-order="<?= esc($f) ?>"><?= date('d/m/Y',strtotime($f)) ?></td>
            <td><?= esc(implode(' / ', $recintos)) ?: '<span class="text-muted">—</span>' ?></td>
