...
$action        = $isEdit ? base_url('/inspecciones/contingencia-limpieza-desinfeccion/update/') . $inspeccion['id'] : base_url('/inspecciones/contingencia-limpieza-desinfeccion/store');
$storageKey    = $isEdit ? 'cont_limpieza_desinfeccion_draft_' . $inspeccion['id'] : 'cont_limpieza_desinfeccion_draft_new';
$empresaFumigadora = $isEdit ? ($inspeccion['empresa_limpieza'] ?? '') : '';
?>

<h5 class="mb-3">
    <i class="fas fa-spray-can-sparkles me-2"></i>
    <?= $isEdit ? 'Editar' : 'Nuevo' ?> Plan de Contingencia — Limpieza y Desinfección
</h5>
...
<form id="limpiezaForm" action="<?= $action ?>" method="post">
    ...
    <div class="card mb-3">
        <div class="card-header" style="background: #1c2437; color: white;">
            <i class="fas fa-broom me-1"></i> Empresa Prestadora del Servicio
        </div>
        <div class="card-body">
            <label class="form-label fw-bold">Nombre y contacto de la empresa prestadora del servicio</label>
            <textarea name="empresa_limpieza" class="form-control" rows="3"
                      placeholder="Ej: Aseo Total S.A.S. — Tel: 311 000 0000 — Contacto: Juan Pérez"><?= esc($empresaFumigadora) ?></textarea>
            <small class="text-muted">Este dato aparecerá en el plan de contingencia (FT-SST-234).</small>
        </div>
    </div>
    ...
    <div class="card mb-3">
        <div class="card-body">
            <p class="text-muted mb-0" style="font-size: 13px;">
                ...
                (Plan de Contingencias Limpieza y Desinfección) con el protocolo de actuación completo.
            </p>
        </div>
    </div>
...
function restoreFromLocal(data) {
    ...
    if (data.empresa_limpieza) document.querySelector('[name="empresa_limpieza"]').value = data.empresa_limpieza;
...
initAutosave({
    formId: 'limpiezaForm',
    storeUrl: base_url('/inspecciones/contingencia-limpieza-desinfeccion/store'),
    updateUrlBase: base_url('/inspecciones/contingencia-limpieza-desinfeccion/update/'),
    editUrlBase: base_url('/inspecciones/contingencia-limpieza-desinfeccion/edit/'),
