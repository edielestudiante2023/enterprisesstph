<div class="container-fluid px-3">
    ...
    <a href="<?= base_url('/inspecciones/dotacion-todero/create') ?>" class="btn btn-pwa btn-pwa-primary mb-3">
    ...
    <?php if (empty($inspecciones)): ?>
        <div class="text-center text-muted py-4">
            <i class="fas fa-broom" style="font-size:40px; opacity:0.3;"></i>
            <p class="mt-2">No hay inspecciones de dotacion todero</p>
        </div>
    <?php else: ?>
        ...
        <div class="card card-inspeccion mb-2 insp-card" data-cliente="<?= esc($insp['nombre_cliente'] ?? '') ?>">
            <div class="card-body py-2 px-3">
                ...
                <div class="mt-2 d-flex gap-1 flex-wrap">
                        <a href="<?= base_url('/inspecciones/dotacion-todero/edit/') ?><?= $insp['id'] ?>" class="btn btn-sm btn-outline-dark">
                    ...
                    <?php if ($insp['estado'] === 'completo'): ?>
                        <a href="<?= base_url('/inspecciones/dotacion-todero/view/') ?><?= $insp['id'] ?>" class="btn btn-sm btn-outline-dark">
                            <i class="fas fa-eye"></i> Ver
                        </a>
                        <a href="<?= base_url('/inspecciones/dotacion-todero/pdf/') ?><?= $insp['id'] ?>" class="btn btn-sm btn-outline-primary" target="_blank">
    ...
    document.addEventListener('click', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '<?= base_url('/inspecciones/dotacion-todero/delete/') ?>' + btn.dataset.id;
            }
        });
    });
});
