...
<head>
  ...
  <style>
    ...
    /* Estilo para el icono de fila expandible */
    td.details-control {
      text-align: center;
      cursor: pointer;
    }
...
<body>
    ...
    <div class="table-responsive">
      <table id="cronogramaTable" class="table table-bordered table-hover" style="width:100%">
        <thead class="text-center">
          ...
          <!-- Segunda fila: filtros con select o input -->
          <tr>
            <!-- Celda vacía para la columna de detalle -->
            <th></th>
            <!-- Para el ID se puede filtrar por exacto o dejar vacío -->
            ...
            <th><input type="text" class="form-control form-control-sm column-search" placeholder="Filtrar fecha"></th>
            <!-- Para columnas con opciones predefinidas se puede usar un select -->
            <th>
            ...
            <th><!-- No se filtra la columna de acciones --></th>
          </tr>
        </thead>
        <tbody>
          <?php if (!empty($cronogramas) && is_array($cronogramas)): ?>
            <?php foreach ($cronogramas as $cronograma): ?>
              <tr data-id="<?= esc($cronograma['id_cronograma_capacitacion']) ?>">
                <!-- Celda para el control de fila expandible -->
  ...
  <script>
    // Función para mostrar los detalles de la fila (child row)
    function format(rowData) {
      // Puedes personalizar la información a mostrar. En este ejemplo se listan todos los datos de la fila.
      var details = '<table class="table table-sm table-borderless">';
      details += '<tr><td><strong>ID:</strong></td><td>' + rowData[1] + '</td></tr>';
      details += '<tr><td><strong>Capacitación:</strong></td><td>' + rowData[2] + '</td></tr>';
      details += '<tr><td><strong>Enfoque:</strong></td><td>' + rowData[3] + '</td></tr>';
      details += '<tr><td><strong>Cliente:</strong></td><td>' + rowData[4] + '</td></tr>';
      // Agrega aquí más filas con la información que desees mostrar...
      details += '</table>';
      return details;
    }

    $(document).ready(function () {
      // Inicialización de DataTable con botones y configuración
      var table = $('#cronogramaTable').DataTable({
        ...
        order: [[1, 'asc']],
        // Ajustamos la columna 0 (fila expandible) para que no se ordene y se use para el control
        columnDefs: [{
          targets: 0,
          orderable: false,
          searchable: false,
      ...
      // Evento para la búsqueda global
      $('#globalSearch').on('keyup', function () {
        table.search(this.value).draw();
      });

      // Evento para filtrar por cada columna
      $('.column-search').on('keyup change', function () {
        var index = $(this).parent().index();
        table.column(index).search(this.value).draw();
      });

      // Restablecer filtros (búsqueda global y por columnas)
      ...
      // Evento para expandir o contraer la fila
    ...
    // Función para enviar datos al servidor (edición en línea)
